...
function preCarga() {
  ...
  if (modelo != "boom") {
    ...
    // sampler[4] = ["+0.84", "+0.35", "+0.63"]    //Para cambiar las duracions?? bug
    sampler_set[5] = 0; if (modelo == "rain") sampler_set[5] = 1 // tipo de sample
    poly_set[3] = random(0.1, 0.15) // grano  , rampa a agudos mas lenta (menos valor -> mas lenta la rampa)
  }
  else {
    ...
    poly_set[3] = random(0.1, 0.25) // // grano  , rampa a agudos mas rapida
  }
...
function jugando() {
  ...
  if (mi_frameCount > -1) {
    if (mi_frameCount % lim_fC[0] == lim_fC[1]) {
      if (pos_n < c_years) genera_cajas()
      reduce_cajas((pos_n - 1) % 4)
    }
    else if (mi_frameCount % lim_fC[0] == lim_fC[2] ||
      mi_frameCount % lim_fC[0] == lim_fC[3]) { reduce_cajas((pos_n - 1) % 4) }
    else if (mi_frameCount % lim_fC[0] == lim_fC[4]) {
...
function suena_armo(_d) { // sampler + poly
  ...
  if (repe_cont % sampler_set[0] == 0 && pos_n < (c_years - 3)) {
    ...
    if (b_rect) {
      sampler[sampler_set[5]].triggerAttackRelease(notas_c[melodia[0]], _d, Tone.now(), 0.24)// 0.09
      let _n = notas_c[melodia[1] + 12]
      if (sound_random() < desaf + 0.2) _n = desafina(melodia[1] + 12)
      sampler[sampler_set[5]].triggerAttackRelease(_n, _d, "+0.02", 0.066)// "+0.35", 0.037) //+0.15     0.2
      ...
      sampler[sampler_set[5]].triggerAttackRelease(_n, _d, "+0.04", 0.05)//"+0.63", 0.024) // +0.23     0.2
    }
  }
}
...
function suena_memb2(_d, _v) { // sinte membrana
  ...
  membrana.set({ envelope: { sustain: _r, release: memb_set[0][repe_cont % 9] } })
...
function genera_cajas() {
  ...
  // codigos --------------------------------
  for (let i = 0; i < 5; i++) {
    cod_sel[i] = cod_car[int(txt_random() * cod_car.length)]
  }
  if (modelo != "boom") {
    gral_cont = 0 // reset
    frame_cont = 0
...
function elije_caja() {
  ...
  if (_te >= 2) _i = cajas.length // 100 hasta 2 textos en total (super bug: antes 100, ahora cajas.length)

  while (_i < cajas.length - 1) { // antes< 99
    const _c = int(txt_random() * cajas.length)// int(random(cajas.length))
    _i = cajas[_c].caja_texto(_i, cajas.length) //(antes solo _i)
    _i++
  }
}
...
function busca_color(x, y) {

  const _x = int(x), _y = int(y)
  const _d = pixel
...
function fin_todo() {
  ...
  membrana.dispose() // ⭐ Detén la membrana
  
  // Detén Tone.js completamente
  Tone.Transport.stop()
...
class Caja {
        ...
        if (this.desN <= p_dCant) {
          ...
          if (random() > _rC) {
            ...
            if (!_bo) {
              ...
              const _n = (sin((mi_frameCount + pos_sin) * pos_sL) + 1) * pos_sA
  ...
  caja_texto(_i, _n) { // si es grande, hace gestos
    if (this.ac && this.w > _n && this.h > _n) {//this.w > 100 && this.h > 100) {
      this.bTexto = true
      return _n//100
    }
    else return _i
