...
//https://shnayto.github.io/harmony-of-the-spheres/
// https://observablehq.com/@esperanc/tone-js-instruments
// 
// https://tonejs.github.io/docs/15.1.3

// https://editor.p5js.org/mrbombmusic/sketches/bY4V2tn4j


// prueba celu falta hacer
...
let bFin = false
let pixel = 1
let cajas = [], cajas_pos = [], cajas_n = 0//, cajas_ac_n = 0
...
/* let pg_grilla, gr_nX = 0, gr_nY = 0, b_grilla = false, gr_image
let patr_mo = 0
let pa_x = 1000, pa_y = 1000
let pa_w = 250, pa_h = 250
let pa_ta = [4, 4, 4, 15, 3, 3, 12, 7] */
...
let color_get = []
////const hit_poly = [[], []]; // stores the vertices for our polygon.

let frame_cont = 0
let mov_vel
let solX, solY
p5.disableFriendlyErrors = false // bug


let sound_ON = false, soundR = [], sound_cont = 0
let channel
let poly, panPo, reverb
let poly_cont = 0, poly_set = []

let sampler, sampler_set = [], sampler_loaded = false, feedbackDelay, panSa
const escala = [36, 38, 39, 41, 43, 46, 48, 50]// [36, 36, 38, 40, 43, 45, 47, 48]// 
let melodia = [], melo_cont = -1, intervalo_ar, melo_sig = -1
let subdivision

let pluck, panSi, intervalo_pl, repe_cont3 = -1
let pluck_set = [
...
function preload() {
  channel = new Tone.Channel({ volume: 0, channelCount: 2 })
  panPo = new Tone.Panner(0)
  panSi = new Tone.Panner(0)
  ...
  //const reverb1 = new Tone.Reverb({ decay: 2, wet: 1 })
  //delay = new Tone.Delay({ delayTime: "+2", maxDelay: "+10" })//"4n")
  /* const dist = new Tone.Distortion({//0.8)
    distortion: 0.8,
    //oversample: OverSampleType;
    wet: 1
  }) */
  //const filter = new Tone.Filter(100, "highpass")
  const comp = new Tone.Compressor({
    ratio: 12, threshold: -20, release: 0.25, attack: 0.003, knee: 3
  })

  feedbackDelay = new Tone.FeedbackDelay({ wet: 0.5 })
  const vol_final = new Tone.Volume(2)

  poly = new Tone.PolySynth()
  poly.set({
    volume: -6,
    envelope: {
      attack: 0, decay: 0, sustain: 0.01, release: 0.001,
      releaseCurve: [1, 0]
    },
    oscillator: {
      type: "triangle16" //"sawtooth16" //"sine6"//  //"square16"
    },
  })
  ...
  sampler.chain(panSa, feedbackDelay, channel)

  pluck = new Tone.PluckSynth()
  pluck.set({
    //attackNoise: 2,
    //envelope: {
    //attack: 0.05, decay: 0.01, sustain: 0.1, release: 0.1,
    //releaseCurve: [1, 0]
    //},
  })
  pluck.chain(panSi, channel)
  ...
  membrana.chain(filter, channel); //dist 
  ...
  // visual setting ------------------
  let _rS = random()
  if (_rS < 0.5) { mi_width = 2160, mi_height = 2160 } //4x3 = 2160x2880
  ...
  if (modelo == "boom") { // modelo boom -----------
    ...
    for (let i = 0; i < 80; i++) { // 40
      pos_fX[i] = _pX * cos((i + pos_ini) * 0.31) + mi_width * 0.25; //cos((i + pos_ini) * 0.31)
    ...
    for (let i = 0; i < 80; i++) {
      ...
      pos_fX[i] = _pX * cos((i + pos_ini) * 0.2) + mi_width * 0.25 //cos((i + pos_ini) * 0.2)
  ...
  dias_memo = dias
  if (dias != 15) pos_gralP = 0.998; else pos_gralP = 0.999


  // sound setting -------------------
  bpmN = int(random(27, 33)); if (random() < 0.75) bpmN += 10 // 40
  Tone.Transport.bpm.value = bpmN * 2

  subdivision = random(["8n", "8t", "16n", "16n.", "16t", "32n", "32n."]) // duplicar "8n" bug
  ...
  // fxhash features ---------------------
  const _cH = col_H[col_Hn] + col_ra / 2
...
function setup() {
  ...
  ////pg_titulo.imageMode(CENTER)
  //pg_titulo.rectMode(CORNER)
  //pg_titulo.strokeWeight(10)
  //pg_titulo.fill(((180) % 360), backS, backB, 0.5)
  ...
  if (seccion == "cargando") {
    ...
    if (frameCount > -1) {
      if (frameCount % lim_fC[0] == lim_fC[1]) {
        ...
        reduce_cajas((pos_n - 1) % 4)
        ajusta_limite()
        ////if (pos_n == c_years) bFin = true bug
      }
    }
...
function suena_poly() { // sinte Poly
  if (sound_ON) {
    ...
    //poly.triggerAttackRelease(random(80, 6000), 0.001, Tone.now(), random(0.03, 0.3))
    let _n
    ...
    let _v = map(_re, 0.3, 0.9, 0.2, 0.008) // 0.03 + sound_random() * 0.32
    poly.triggerAttackRelease(_n, 0.001, Tone.now(), _v) // dur: 0.01
  }
}
...
function suena_armo(_d) { // sampler + poly
  if (sound_ON) {
    ...
    if (repe_cont3 % sampler_set[0] == 0 && pos_n < (c_years - 3)) {
      ...
      let _r1 = 0.8; if (seccionM == "B") _r1 = 0.4
      if (sound_random() < _r1) feedbackDelay.set({ delayTime: _r, feedback: 0.99 })//
      else {
        ////print("-frena")
        feedbackDelay.set({ delayTime: 0.1, feedback: 0.9 }) // bug
        let _n = notas_c[melodia[1] - 19]
        poly.triggerAttackRelease(_n, 5, "+0.89", 0.035) // 0.02
        _d = 3
      }

      sampler.triggerAttackRelease(notas_c[melodia[0]], _d, Tone.now(), 0.09)// 0.25
      let _n = notas_c[melodia[1] + 12]
      if (sound_random() < desaf + 0.2) _n = desafina(melodia[1] + 12)
      sampler.triggerAttackRelease(_n, _d, "+0.02", 0.039)// "+0.35", 0.037) //+0.15     0.2
      _n = notas_c[_m + sampler_set[1]]  //notas_c[melodia[2] + sampler_set[1]]
      ////print("_n " + _n, sampler_set[1])
      if (sound_random() < desaf + 0.2) _n = desafina(_m + sampler_set[1])//melodia[2] + sampler_set[1])
      sampler.triggerAttackRelease(_n, _d, "+0.04", 0.026)//"+0.63", 0.024) // +0.23     0.2
    }
  }
}
...
function suena_pluck(_ri, _fr) {
  if (seccionM == "B") repe_cont3++ //orig

  if (sound_ON) {
    panSi.set({ pan: -1 + sound_random() * 2 }) // random(-1, 1) })
    let _re, _vo
    if (sound_random() > pos_n * 0.004) {
      _re = pluck_set[1][repe_cont3 % pluck_res]; _vo = pluck_set[2][repe_cont3 % 4]
    }
    else {
      _re = 0.1 + sound_random() * 0.8; _vo = pluck_set[2][repe_cont3 % 4] * 0.1
    }
    pluck.set({ resonance: _re })
    pluck.set({ volume: _vo })

    if (poly_cont == 0) { pluck.triggerAttack(notas_c[(_ri + 24) % 120], Tone.now()) } //_ri + 12
    else {
...
function suena_memb(_d, _v) { // sinte membrana

  if (sound_ON) {
    ////print("bajo " + notas_c[melodia[melo_cont % melodia.length]])
    let _su = 0.15; if (sound_random() < 0.5) _su = 0.001  //if (repe_cont3 % 64 == 32) _su = 0.001
    membrana.set({ envelope: { sustain: _su, release: 0.3 } })
    membrana.triggerAttackRelease(notas_c[melodia[melo_cont % melodia.length]], _d, "+0.01", _v)
  }
}
...
function suena_memb2(_d, _v) { // sinte membrana
  ...
  if (sound_ON) {
    ...
    membrana.set({ envelope: { sustain: _r, release: memb_set[0][repe_cont3 % 9] } })
    let _n = int(noise(0.1 * repe_cont3) * 36) + 36//* 48) + 36

    if (pos_n % 4 == 2) poly.triggerAttackRelease(notas_c[_n - 12 % 120], 0.25, "+0.01", 0.025)
    membrana.triggerAttackRelease(notas_c[_n], _d, Tone.now(), _v)
  }
}
...
function Rep_A(_r) { // loop asimetrico // bug cuidado no para con key "p"
  ...
  intervalo_A = setInterval(() => {
    ...
    const _i = repe_cont3 % _lo// ritmo[0].length// - (_nn - 1)
...
function arma_melo(_v) {
  ...
  for (let i = 0; i < _r; i++) {
    melodia[i] = escala[int(gral_random() * escala.length)] + _v
  }
...
function arma_ritmo(_t) { // cambiar todo a gral_sound?
  ...
  if (gral_random() < 0.5) {
    ...
    _cn = 0.33; pluck_set[0] = abs(pluck_set[0])
  } // orig: 0.66, 0.01
  else { _cn = 1 } //, _cnC = -0.02 // cantidad notas // orig= -0.01
...
function seccionA() {
  ...
  if (sound_ON) pluck.triggerRelease(Tone.now()) // bug
  repe_cont3 = -1 // bug?
  poly_cont = 0
...
function genera_cajas() {
  ...
  if (pos_n < c_years) {
    ...
    else if (modelo == "rain") _rD = 3; else _rD = random([1, 3]) //1= es para arriba
    ...
    for (let xx = 0; xx < dias; xx++) {
      for (let yy = 0; yy < dias; yy++) {
        ...
        if (gral_random() < 0.07) _ac = false //|| xx != 5 && yy != 5
...
function fin_todo() {
  //print("fin todo")
  if (sound_ON) {
    feedbackDelay.set({ delayTime: 0, feedback: 0 })
    clearInterval(intervalo_A) // frena seccion loop asimetrico
    clearInterval(intervalo_pl) // bug
...
function click() {
  ...
  if (!sound_ON && seccion == "jugando" && (millis() % 3000) < 1500) {
    push()
...
/* function loop_random() {
  loop_cont++
  if (this.loop_cont == 1000) loop_cont = 0
  return loopR[loop_cont]
} */
...
function mouseClicked() {
  if (sampler_loaded) {
    ...
    sound_ON = !sound_ON
    if (!sound_ON) feedbackDelay.set({ delayTime: 0, feedback: 0 })
    ////else suena_armo(0.5)
  }
}
function grabaImagen() {
  console.log("saving!")
  saveCanvas("nuevo" + mi_seed + ".png")
}
...
class Caja {
  constructor(_ac, _x, _y, _w, _h, _cL, _xx, _yy, _pos_n, _oI) {
    ...
    ////let _in = inicioVel * 1000 // 0.5 * 800
    ////_in = constrain(_in, 1, 900)

    this.inicio = _oI //  orden inicio

    this.backH = (col_H[col_Hn] + (gral_random() * col_ra)) % 360//random(col_H[col_Hn], col_H[col_Hn] + 20)
    //random(180, 220)//azules // random(10, 50) //(this.pos_n * 30) * int(random(6)) * 1;
    if (random() < 0.95) { // bug gral_random?????
      ...
      //col_gral[0] = int(this.loop_random() * 2) * 70// int(random(2)) * 70 // 0 o 70
      //col_gral[1] = int(this.loop_random() * 2) * 70//int(random(2)) * 70
  ...
  dibuja() {
    //// if (this.posX == 0 && this.posY == 0) print(this.inicio, pos_dist[this.pos_n])
        ...
        if (this.desN <= p_dCant) {
          ...
          if (random() > _rC) {
            ...
            if (!_bo) {
              ...
              if (this.col == 255 && (this.desN < _n || random() < 0.05)) {//< 0.2
          ...
          if (cajas.length < 20) fin_todo()////if (bFin) fin_todo() bug
        }
      }
    }
    this.vida--
